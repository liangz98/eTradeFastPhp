...
class SettleController extends Kyapi_Controller_Action {
    ...
    public function preDispatch() {
        ...
        // 结算中心, 先验证当前帐号是否已登录
        $hasInitPaymentPwd = $this->json->paymentgetAccountUserApi($this->_requestObject);
        if (json_decode($hasInitPaymentPwd)->result->userStatus == '00') {
            if (!strpos($_SERVER['REQUEST_URI'], 'initpwd')) {
                $resultMsg = '00';
                $this->redirect("/settle/initpwd?resultMsg=" . $resultMsg);
            }
        }
    }
    ...
    public function listviewAction() {
        try {
            ...
            if ($existDatt['result']['rptype'] == "R") {
                $this->view->t1 = "+";
            } else if ($existDatt['result']['rptype'] == "R") {
                $this->view->t1 = "-";
            }
        } catch (Exception $e) {
            Shop_Browser::redirect($e->getMessage());
    ...
    public function initpwdAction() {
        if (!empty($this->_request->getParam('resultMsg')) && $this->_request->getParam('resultMsg') == '00') {
            $this->view->resultMsg = $this->view->translate('tip_payment_pwd');
        }
