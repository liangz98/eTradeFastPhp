<?php
 class Zend_View_Helper_ShowDictionaryList extends Shop_View_Helper
{
	...
	function ShowDictionaryList($files_name,$lang_code,$e,$t){
        ...
        foreach($str as $k=>$v){
            if(!empty($t)){
            if($t==$v['code']){
                $dclist.= '<option selected="selected" value='.$v['code'].'>'.$v['code'].$v['name'].'</option>';
            }else{
                $dclist.= '<option value='.$v['code'].'>'.$v['code'].$v['name'].'</option>';
            }}
            else{
                if($lang_code=="zh_CN"){
                if($v['code']=="CN"){
