<?php
class Zend_View_Helper_ShowWebuploaderT1 extends Shop_View_Helper
{
	function ShowWebuploaderT1($attArr,$bizType,$attachType,$type,$goodsID)
	{
		//$attArr 服务端返回附件信息数组；
       //$goodsID   绑定业务ID
		//$bizType 业务类型
		...
		$str="";
		$indata="";
		$indata.='<input  type="hidden" id="bizTypeT1" value='.$bizType;
		$indata.='><input  type="hidden" id="bizIDTT1" value='.$goodsID;
		$indata.='><input  type="hidden" id="attachTypeT1" value='.$attachType;
		$indata.='><input  type="hidden" id="uploadURLT1" value='.$this->view->seed_Setting['KyUrlex'];
		$indata.='><input  type="hidden" id="typeURLT1" value='.$type;
		$indata.='>';
