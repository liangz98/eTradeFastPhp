...
class Zend_View_Helper_GoodsSearchAttr extends Shop_View_Helper
{
	...
	public function init()
	{
		...
		if(empty($this->attributes)){
			...
			if(!empty($this->module)){
				...
				$this->urlSpec = preg_replace('/{module}/', "", $this->urlSpec);
			}
			...
			if(!empty($this->activeUrl)){
				...
				if(isset($matches['1']) && !empty($matches['1'])){
					...
					if(!empty($this->module))
						...
						$this->regex = '/^\/' . $this->controllerName . '\/' . implode('-', $_regex) . '.html/isU';
				}
			}else{
	...
	public function getRegexShort()
	{
		if(!empty($this->module))
			...
			$regex_short = '/^\/' . $this->controllerName . '\/index.html/isU';
		return $regex_short;
	}
	...
	public function getInitUrlByLong()
	{
		if(!empty($this->module))
			...
			return $this->controllerName . '/' .implode('-', $this->matches['default']). '.html';
	}
	...
	public function getInitUrl()
	{
		if(!empty($this->module))
			...
			return $this->controllerName . '/index.html';
	}
