<?php
class GroupsController extends Shop_Controller_Action
{
	public $perpage = 20;
	public $goods_total = '';
	public $page_num = '';
	public function preDispatch() {
        ...
        if (isset($arr[1]) && !empty($arr[1])) {
            ///入口列表
            preg_match_all('/^\/groups\/(top)-([\d]+).html/isU', $cururl, $arr);
            ...
            ///入口列表 
            preg_match_all('/^\/groups\/([\w]+)-([\d]+)-([\d]+).html/isU', $cururl, $arr);
            if (isset($arr[1][0]) && !empty($arr[1][0])) {
                $this->_request->setParam('mod', $arr[1][0]);
                $this->_request->setParam('cate_id', $arr[2][0]);
            ...
            ///库存详细入口
            preg_match('/^\/groups\/([\w]+)(-[\w]+)?.html/isU', $cururl, $arr);
            $group_id = 0;
            if (isset($arr[1]) && !empty($arr[1])) {
                if (count($arr) == 3) {
                    if ($arr[2]{0} == '-') {
            ...
            ///商品详细
            preg_match_all('/^\/groups\/([\w]+).html/isU', $cururl, $arr);
            if (isset($arr[1][0]) && !empty($arr[1][0])) {
            ...
            preg_match_all('/^\/groups\/(sold|hot|top|price|new)-([\d]+)-([\d]+)-([-\d]+).html/isU', $cururl, $arr);
            if (isset($arr [4] [0])) {
                //根据分类找出属性
                $cate_id = $arr [2] [0];
    ...
    public function indexAction()
	{
		...
		$mod = "top";
		if (empty($cate_id)){
			$goodscate = $goodsCateM->fetchRow(array('is_actived'=>'1','is_group'=>'1'));
			$cate_id = $goodscate['cate_id'];
		}
		$goodscate = $goodsCateM->fetchRow(array('cate_id'=>$cate_id,'is_group'=>'1'));
		...
		foreach ($goodses as $k =>$goods){
			$goodsgroups = $goodsGroupM->fetchRows(null,array('group_id'=>$goods['goods_id']));
			if ($goodsgroups){
				$shop_price = '0';
				$cost_price = '0';
				foreach ($goodsgroups as $gk=>$gv){
					...
					if($stock['stock_id']>0){
						$goodses[$k]['groups'][$gk]['goods_mark'] = $single_goods['goods_mark'];
						$goodses[$k]['groups'][$gk]['goods_list_image'] = $single_goods['goods_list_image'];
						$goodses[$k]['groups'][$gk]['goods_number'] = $gv['goods_number'];//单个商品的数量
						$goodses[$k]['groups'][$gk]['shop_price'] = $gv['shop_price'];//单个商品的现价
						$goodses[$k]['groups'][$gk]['cost_price'] = $stock['stock_market_price'];//单个商品的原价
						
						$shop_price +=($gv['shop_price']*$gv['goods_number']);//总的,现价
					    $cost_price +=($stock['stock_shop_price']*$gv['goods_number']);//总的,原价
					}else{
						unset($goodses[$k]['groups'][$gk]);
//						$goodses[$k]['groups'][$gg]['goods_number'] = '0';
//						$goodses[$k]['groups'][$gg]['shop_price'] = '0.00';
//						$goodses[$k]['groups'][$gg]['market_price'] = '0.00';
					}
				}
		...
		if(defined('SEED_WWW_TPL')){
			$content = $this->view->render(SEED_WWW_TPL."/groups/index.phtml");
			echo $content;
			exit;
		}
	}
