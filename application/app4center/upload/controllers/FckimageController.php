<?php
class FckimageController extends Seed_Controller_Action4Admin 
{
	...
	function browserAction(){
		...
		if(count($images)==0){
			Seed_Browser::redirect("没有图片选择",$this->view->seed_BaseUrl."/fckimage/upload?&name=".$name."&token=".$this->view->seed_Token);
		}
		$this->view->name=$name;
		...
		if ($this->_request->isPost()) {
			try{
				...
				if (empty($insertData['cate_name'])){
					...
					if($check['path_id']>0){
						Seed_Browser::redirect('目录已经存在！',$this->view->seed_BaseUrl.'/coverimage/upload?name='.$insertData['cate_name'].'&path='.$insertData['path_name']);
					}
					if($path_id = $catepathM->insertRow($insertData)){
						Seed_Browser::redirect('添加目录成功！',$this->view->seed_BaseUrl.'/coverimage/upload?name='.$insertData['cate_name'].'&path='.$insertData['path_name']);
					}else{
