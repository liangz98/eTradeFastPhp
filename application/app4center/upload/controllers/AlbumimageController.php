<?php
class AlbumimageController extends Seed_Controller_Action4Admin 
{
	function indexAction()
	{
		...
		$field=$f1->filter($this->_request->getParam('field'));
		$from=$f1->filter($this->_request->getParam('from'));
		...
		if(count($images)==0){
			Seed_Browser::redirect("没有图片选择",$this->view->seed_BaseUrl."/albumimage/upload?&name=".$name);
		}
		$this->view->name=$name;
	...
	function selectAction(){
		...
		if ($this->_request->isPost()) {
			try{
				$selected_images = $this->_request->getPost('selected_images');
				$selects=array();
				if(count($selected_images)>0){
					$imageM = new Seed_Model_Image('system');
					$images = $imageM->fetchRowsByIds('image_id',$selected_images);
					foreach ($images as $k=>$image){
						$selects[$k]['image_id']=$image['image_id'];
						$selects[$k]['cate_path']=$image['cate_path'];
						$selects[$k]['original_path']=$image['original_image_path'];
						$selects[$k]['normal_path']=$image['normal_image_path'];
						$selects[$k]['thumb_path']=$image['thumb_image_path'];
					}
				}
		...
		if ($this->_request->isPost()) {
			try{
				...
				if (empty($insertData['cate_name'])){
					...
					if($check['path_id']>0){
						Seed_Browser::redirect('目录已经存在！',$this->view->seed_BaseUrl.'/albumimage/upload?name='.$insertData['cate_name'].'&path='.$insertData['path_name']);
					}
					if($path_id = $catepathM->insertRow($insertData)){
						Seed_Browser::redirect('添加目录成功！',$this->view->seed_BaseUrl.'/albumimage/upload?name='.$insertData['cate_name'].'&path='.$insertData['path_name']);
					}else{
