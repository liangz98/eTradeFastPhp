<?php
class MailtemplateController extends Seed_Controller_Action4Admin
{
	function indexAction()
	{
		$smsTempM = new Seed_Model_MailTemplate('system');
			...
			try{
				...
				if(empty($insertData['temp_name'])){
					...
					if($temp_id = $smsTempM->insertRow($insertData)){
						Seed_Browser::tip_show('添加成功！',$this->view->seed_BaseUrl."/mailtemplate/index");
		    		}else{
			...
			try{
				...
				if($temp_id<1){
					...
					if($smsTempM->updateRow($updateData,array('temp_id'=>$temp_id))){
						Seed_Browser::tip_show('修改成功！',$this->view->seed_BaseUrl."/mailtemplate/index");
		    		}else{
	...
	function deleteAction() {
		...
		if ($this->_request->isPost()){
			try{
				...
				Seed_Browser::tip_show('删除成功！',$this->view->seed_BaseUrl.'/mailtemplate/index');
				exit;
			} catch (Exception $e) {
				Seed_Browser::tip_show($e->getMessage());
			}
