<?php
class MobiletemplateController extends Seed_Controller_Action4Admin
{
	function indexAction()
	{
		$smsTempM = new Seed_Model_MobileTemplate('system');
		$temps = $smsTempM->fetchRows();
	...
	function addAction()
	{
		if ($this->_request->isPost()) { 
			try{
				...
				$insertData['send_to']=$f5->filter($this->_request->getPost('send_to'));
				...
				if(empty($insertData['temp_name'])){
					...
					$smsTempM = new Seed_Model_MobileTemplate('system');
					if($temp_id = $smsTempM->insertRow($insertData)){
						Seed_Browser::tip_show('添加成功！',$this->view->seed_BaseUrl."/mobiletemplate/index");
		    		}else{
	...
	function updateAction()
	{
		if ($this->_request->isPost()) { 
			try{
				...
				$tempDetail = $smsTempM->fetchRow(array('temp_id'=>$temp_id));
				...
				if($temp_id<1){
					...
					if($smsTempM->updateRow($updateData,array('temp_id'=>$temp_id))){
						Seed_Browser::tip_show('修改成功！',$this->view->seed_BaseUrl."/mobiletemplate/index");
		    		}else{
		...
		$temp = $smsTempM->fetchRow(array('temp_id'=>$temp_id));
	...
	function deleteAction() {
		...
		if ($this->_request->isPost()){
			try{
				...
				foreach ($temp_ids as $temp_id){
					...
					if($temp_id>0){
						$smsTempM->deleteRow(array('temp_id'=>$temp_id));
					}
				}
				Seed_Browser::tip_show('删除成功！',$this->view->seed_BaseUrl.'/mobiletemplate/index');
				exit;
			} catch (Exception $e) {
				Seed_Browser::tip_show($e->getMessage());
			}
		...
		if(count($mytemps_arr)>0){
			$smsTempM = new Seed_Model_MobileTemplate('system');
			$temps = $smsTempM->fetchRowsByIds('temp_id',$mytemps_arr);
	   		$this->view->temps = $temps;
		}
	}
