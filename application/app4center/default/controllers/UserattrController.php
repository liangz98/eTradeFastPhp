<?php
class UserattrController extends Seed_Controller_Action4Admin 
{
	...
	if ($this->_request->isPost())
		{
			try 
			{
				...
				if (empty($insertData['attr_name']))
				{
					...
					$userAttrM = new Seed_Model_UserAttr('system');
					$attr_id = $userAttrM->insertRow($insertData);
					if ($attr_id>0)
					{
						$profileM = new Seed_Model_Profile('system');
						$check=$profileM->fetchRowBySql("describe ".$profileM->getTableName()." ".$insertData['field_name']);
						if(!$check){
							if(!empty($insertData['field_name']) && $insertData['field_type']=="varchar" && $insertData['field_length']>0){
								$profileM->query("ALTER TABLE `".$profileM->getTableName()."` ADD `".$insertData['field_name']."` ".$insertData['field_type']."( ".$insertData['field_length']." ) NULL COMMENT '".$insertData['attr_name']."';");
							}elseif(!empty($insertData['field_name']) && $insertData['field_type']=="text"){
								$profileM->query("ALTER TABLE `".$profileM->getTableName()."` ADD `".$insertData['field_name']."` ".$insertData['field_type']." NULL COMMENT '".$insertData['attr_name']."';");
							}
						}
						
						Seed_Browser::tip_show("添加数据成功!", $this->view->baseUrl().'/userattr/index');
					}
					else 
					{
	...
	function updateAction()
	{
		if ($this->_request->isPost()) { 
			try{
				...
				if (empty($updateData['attr_name']))
				{
					...
					$userAttrM = new Seed_Model_UserAttr('system');
					$rs = $userAttrM->updateRow($updateData, array('attr_id' => $attr_id));
					if ($rs)
					{
						Seed_Browser::tip_show("编辑数据成功！", $this->view->baseUrl().'/userattr/index');
					}
					else 
					{
		...
		$userAttrM = new Seed_Model_UserAttr('system');
		$attribute = $userAttrM->fetchRow(array('attr_id'=>$attr_id));
	...
	function deleteAction() {
			...
			try{
				...
				foreach ($attr_ids as $attr_id){
					...
					if($attr_id>0){
						...
						$attr = $userAttrM->fetchRow(array('attr_id'=>$attr_id));
						//print_r($attr);exit;
						if ($attr['attr_id']>0){
							$profileM->query("ALTER TABLE `".$profileM->getTableName()."`DROP`".$attr['field_name'].";");
						}
						
						$userAttrM->deleteRow(array('attr_id'=>$attr_id));
					}
				}
				
				Seed_Browser::tip_show('删除成功！',$this->view->seed_BaseUrl.'/userattr/index');
				exit;
			} catch (Exception $e) {
				Seed_Browser::tip_show($e->getMessage());
			}
		...
		if(count($myattrs_arr)>0){
			$userAttrM = new Seed_Model_UserAttr('system');
			$attributes = $userAttrM->fetchRowsByIds('attr_id',$myattrs_arr);
			...
			try{
				...
				if(is_array($attr_ids)){
					foreach ($attr_ids as $k=>$attr_id){
						...
						$userAttrM->updateRow($updateData,array('attr_id'=>$attr_id));
					}
				}
				Seed_Browser::tip_show('排序成功！',$this->view->seed_BaseUrl.'/userattr/index');
				exit;
			} catch (Exception $e) {
				Seed_Browser::tip_show($e->getMessage());
			}
