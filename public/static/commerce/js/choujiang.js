$(function($){
    ...
    $('input[name="is_button"]').bootstrapSwitch({
        size: 'small',
        onColor: 'primary',
        offColor: 'danger', 
        onText: '启用',
        ...
        onSwitchChange: function(){
            var $this = $(this), id = $this.attr('data');
             if ( !id) {return false;} 
            $.post(window.location.href, {id:id}, function(op){
                if (op.code == 1) { 
                    if(op.state==1){
//                        var nimei = $this.parents('tr').siblings('.trr').find('input[name="is_button"]');
//                        nimei.each(function(){
...
//                         });
    ...
    $("a.deleteItem").live("click", function(evt){
        ...
        $pop.find('.js-btn-confirm').click(function(){
            ...
            $.getJSON("/b/vmall/draw/delete", {ids:id}, function(op){
                if (op.code == 1) {
      ...
      $("#deleteBtn").click(function(evt){
        ...
        $pop.find('.js-btn-confirm').click(function(){
            ...
            deleting = true;
            $("input[name='chose[]']").attr("disabled", true);
            $.getJSON("/b/draw/delete", {ids:idQuery}, function(op){
                if (op.code == 1) {
