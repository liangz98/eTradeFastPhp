$(document).ready(function($){
    ...
    var setStatus = function($this, v, r, p) {
        ...
        var id = $this.attr("refundid"),
            param = {refund_id:id, refund_status:v, refund_result:r, payment_way:p};
        $.post("/b/vmall/orderrefund2/setstatus", param, function(op){
            if (op.code == 1) {
    ...
    $(".status_0,.status_1").live('click', function(evt){
        ...
        $pop.find('.js-btn-confirm').text(status === 0 ? '接受申请':'确认收货');
        $pop.find('.js-btn-deny').text(status === 0 ? '拒绝申请':'拒绝收货');
    ...
    $(".edit-btn").click(function(evt){
        var $this = $(this),
            $pop = $(popHtml),
            refund_id = $this.parents('table').attr('id').replace(/refund_/, ''),
        ...
        $pop.find('.js-btn-edit').click(function(){
            ...
            $.getJSON("/b/vmall/orderrefund2/update", {id:refund_id, field:field, data:value}, function(op){
                if (op.code == 1) {
