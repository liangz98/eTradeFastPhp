$(function($){
    ...
    $("#goods_id").select2({
        ...
        ajax: {
            url: '/b/vmall/goodspointsmall/goodselect',
            dataType: 'json',
    ...
    $("#deleteBtn").click(function(evt){
        ...
        $pop.find('.js-btn-confirm').click(function(){
            ...
            $.getJSON("/b/vmall/goodspointsmall/delete", {ids:idQuery}, function(op){
                if (op.code == 1) {
    ...
    $("a.deleteItem").live("click", function(evt){
        ...
        $pop.find('.js-btn-confirm').click(function(){
            ...
            $("input[name='chose[]']").attr("disabled", true);
            $this.hide().after('<img class="loading" src="/static/commerce/images/loader.gif">');
            $.getJSON("/b/vmall/goodspointsmall/delete", {ids:id}, function(op){
                if (op.code == 1) {
                    $this.parents("tr").hide('slow', function(){
                        $this.parents("tr").remove();
                    });
    ...
    $("#createBtn").click(function(){
        ...
        var param = {},
            ...
            rowHtml = '<tr style="display:none;"><td><input type="checkbox" name="chose[]" value="" class="ids_all"></td>'+
                      '<td><img src="" style="width:50px;"></td><td></td><td><a class="edit-link" href="javascript:;" field="need_price" data=""></a></td>'+
                      '<td><a class="edit-link" href="javascript:;" field="need_integral" data=""></a></td>'+
                      '<td><a class="deleteItem" data="" href="javascript:;">删除</a></td></tr>',
            $rsDom = $(rsHtml),
            $rowDom = $(rowHtml);
        param.goods_id = goods.goods_id;
        param.stock_id = goods.stock_id;
        param.need_price = parseFloat($("#need_price").val());
        param.need_integral = parseInt($("#need_integral").val());
        if (typeof(goods.goods_id) == "undefined") {
            generate("error", "请选择商品");
            return false;
        }
        if (isNaN(param.need_price) || param.need_price != $("#need_price").val()) {
            generate("error", "请输入所需价格");
            return false;
        }
        if ( !param.need_integral || param.need_integral != $("#need_integral").val()) {
            generate("error", "请输入所需积分");
            return false;
        }
        param.need_price = param.need_price.toFixed(2);
        creating = true;
        $("table.admin_list tr:first").after($rsDom);
        $.post("/b/vmall/goodspointsmall/add", param, function(op){
            if (op.code == 1) {
                $rowDom.attr("id", "pmall_id_"+op.id);
                $rowDom.find("td").each(function(i){
                    switch (i) {
                        ...
                        case 2:
                            ...
                            return true;
                        case 3:
                            $(this).find("a").text(param.need_price).attr("data", param.need_price);
                            return true;
                        case 4:
                            $(this).find("a").text(param.need_integral).attr("data", param.need_integral);
                            return true;
                        case 5:
    ...
    $("a.edit-link").live("click", function(evt){
        var $this = $(this),
            $pop = $(popHtml),
            pmall_id = $this.parents('tr').attr('id').replace(/pmall_id_/, ''),
        ...
        $pop.find('.js-btn-edit').click(function(){
            ...
            if (field == "need_price" && (isNaN(value) || value != $pop.find('input.js-url-placeholder').val())) {
                generate("error", "请输入正确的价格");
                return false;
            }
            if (field == "need_integral" && ( !value || value != $pop.find('input.js-url-placeholder').val())) {
                generate("error", "请输入正确的积分");
                return false;
            }
            if (value == data) {
            ...
            value = (field == "need_price" ? value.toFixed(2) : parseInt(value));
            editing = true;
            $this.hide().after('<img class="loading" src="/static/commerce/images/loader.gif">');
            $.getJSON("/b/vmall/goodspointsmall/update", {id:pmall_id, field:field, data:value}, function(op){
                if (op.code == 1) {
                    $this.text(value);
