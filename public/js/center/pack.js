...
function preview(img, selection) {
	...
	$('.z_img img').css({
        width: Math.round(scaleX2 * imgwidth),
        height: Math.round(scaleY2 * imgheight),
...
$(function () {
	...
	var imgheight = $(".frame img").height();
	var wh = 300;
	if(imgwidth > imgheight && imgwidth > wh){
		$(".frame img").css("width",wh +"px");
	};
	if(imgwidth > imgheight && imgwidth < wh){
		$(".frame img").css("width",imgwidth +"px");
	};
	if(imgheight > imgwidth && imgheight > wh){
		$(".frame img").css("height",wh +"px");
	};
	if(imgheight > imgwidth && imgheight < wh){
		$(".frame img").css("height",imgheight +"px");
	};
	if(imgwidth == imgheight && imgwidth > wh){
		$(".frame img").css("width",wh +"px");
	}
