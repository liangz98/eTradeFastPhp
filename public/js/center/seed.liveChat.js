...
Class.inherit( Ajax.ChatThreadUpdater, Ajax.Base, {
  ...
  postMessage: function(msg) {
    ...
    var postOptions = Object.extend({}, this._options);
  ...
  changeName: function(newname) {
	...
	'&wctoken=' + (this._options.token || 0) + '&name=' + encodeURIComponent(newname)});
  ...
  closeThread: function() {
	var _params = 'act=close&thread=' + (this._options.threadid || 0) + '&wctoken=' + (this._options.token || 0);
	if( this._options.user )
	_params += "&user=true";
...
EventHelper.register(window, 'onload', function(){
  ...
  Chat.threadUpdater = new Ajax.ChatThreadUpdater(Object.extend({ignorectrl:-1,container:myRealAgent=='safari'?self.frames[0]:$("chatwnd"),avatar:$("avatarwnd"),message:$("msgwnd")}, threadParams || {}));
});