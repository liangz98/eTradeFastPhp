...
var HtmlGenerationUtils = {
  ...
  viewOpenCell: function(username,servlet,id,canview,canopen,ban,message,cantakenow) {
		...
		if( canopen ) {
			gen += '<td class="icon">';
			gen += HtmlGenerationUtils.popupLink( link, localized[0], "ImCenter"+id, '<img src="'+mibewRoot+'/images/center/livehelp/tbliclspeak.gif" width="15" height="15" border="0" alt="'+localized[0]+'">', 640, 480, null);
			gen += '</td>';
			cellsCount++;
		}
		if( canview ) {
			gen += '<td class="icon">';
			gen += HtmlGenerationUtils.popupLink( link+"&viewonly=true", localized[1], "ImCenter"+id, '<img src="'+mibewRoot+'/images/center/livehelp/tbliclread.gif" width="15" height="15" border="0" alt="'+localized[1]+'">', 640, 480, null);
			gen += '</td>';
			cellsCount++;
		}
		if( message != "" ) {
  ...
  banCell: function(id,banid){
      return '<td class="icon">'+
          HtmlGenerationUtils.popupLink( mibewRoot+'/operator/ban.php?'+(banid ? 'id='+banid : 'thread='+id), localized[2], "ban"+id, '<img src="'+mibewRoot+'/images/center/livehelp/ban.gif" width="15" height="15" border="0" alt="'+localized[2]+'">', 720, 480, null)+
          '</td>';
  }
};
...
Class.inherit( Ajax.ThreadListUpdater, Ajax.Base, {
  ...
  updateThreads: function(root) {
	...
	this.updateTimers();
	this.setStatus(this._options.istatus ? "离线" : "在线");
	if( newAdded ) {
  ...
  updateOperators: function(root) {
	...
	for( var i = 0; i < root.childNodes.length; i++ ) {
		...
		names[names.length] = 
			'<img src="'+mibewRoot+'/images/center/livehelp/center/livehelp/op'+(isAway ? 'away' : 'online')+
...
EventHelper.register(window, 'onload', function(){
  ...
  new Ajax.ThreadListUpdater(Object.extend({table:$("threadlist"),status:$("connstatus"),istatus:0}, updaterOptions || {}));
