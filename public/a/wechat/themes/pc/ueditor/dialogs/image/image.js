                        ...
                        ajax.request(editor.options.imageManagerUrl, {
                            ...
                            onsuccess:function (xhr) {
                                ...
                                for (var k = 0, ci; ci = imageUrls[k++];) {
                                    ...
                                    delbtn.src = "/images/center/error.gif";
                                    delbtn.setAttribute('delid',ci.substring(parseInt(ci.lastIndexOf('\?')+1))+'');
                                    delbtn.setAttribute('id','delmanage_'+ci.substring(parseInt(ci.lastIndexOf('\?')+1))+'');
                                    delbtn.onclick = function(){
                                        ajax.request(editor.options.imageManagerDelPath,{
                                            data:{
                                                imgid :this.getAttribute('delid')
                                            }
                                            ,onsuccess:function(da){
                                                var d = utils.trim(da.responseText);
                                                if(isNaN(d)){
                                                      alert(d+'');
                                                }else if(document.getElementById('delmanage_'+d)){
                                                    var _parent_node_node = document.getElementById('delmanage_'+d).parentNode.parentNode,
                                                            _parent_node = document.getElementById('delmanage_'+d).parentNode;
                                                    if(_parent_node_node && _parent_node){
                                                            _parent_node_node.removeChild(_parent_node);
                                                    }
                                                }
                                            }
                                            ,onerror:function() {
