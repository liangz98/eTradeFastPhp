...
if (!defined('GZSEED_AUTH_STEP2') || GZSEED_AUTH_STEP2 != '1') {
	...
	//curl_setopt($ch, CURLOPT_URL, 'http://lic.gzscd ..eed123.cn/?from=sendtou&host=' . $_SERVER['HTTP_HOST'] . '&ip=' . $_SERVER['SERVER_ADDR']);
...
if ($registry->isRegistered('db4' . $label)) {
	$this->_db = $registry->get('db4' . $label);
...
try {
	...
	$registry->set('db4' . $label, $this->_db);
