...
class QRimage {
    public static function png($frame, $filename = false, $pixelPerPoint = 4, $outerFrame = 4, $saveandprint = FALSE, $back_color, $fore_color,$width = 0,$height = 0) {
        $image = self::image($frame, $pixelPerPoint, $outerFrame, $back_color, $fore_color,$width,$height);
        if ($filename === false) {
    ...
    private static function image($frame, $pixelPerPoint = 4, $outerFrame = 4, $back_color = 0xFFFFFF, $fore_color = 0x000000,$width = 0,$height = 0) {
        $h = count($frame);
        $w = strlen($frame[0]);
        $imgW = $w + 2 * $outerFrame;
        $imgH = $h + 2 * $outerFrame;
        ...
        if($width > 0 || $height > 0){
            ...
            $imgpW = $imgW * $pixelPerPoint;
            $imgpH = $imgH * $pixelPerPoint;
        }
        $target_image = ImageCreate($imgpW, $imgpH);
        ImageCopyResized($target_image, $base_image, 0, 0, 0, 0, $imgpW, $imgpH, $imgW, $imgH);
...
class QRcode {
    ...
    public function png2($text, $outfile = false, $level = QR_ECLEVEL_L, $size = 3, $margin = 4, $saveandprint = false, $back_color = 0xFFFFFF, $fore_color = 0x000000) {
        $enc = QRencode::factory($level, $size, $margin, $back_color, $fore_color,isset($this->qrWidth)?$this->qrWidth:0,isset($this->qrHeight)?$this->qrHeight:0);
...
class QRencode {
    ...
    public static function factory($level = QR_ECLEVEL_L, $size = 3, $margin = 4, $back_color = 0xFFFFFF, $fore_color = 0x000000,$width = 0,$height = 0) {
        $enc = new QRencode();
        $enc->size = $size;
        $enc->margin = $margin;
    ...
    public function encodePNG($intext, $outfile = false, $saveandprint = false) {
        try {
            ...
            QRimage::png($tab, $outfile, min(max(1, $this->size), $maxSize), $this->margin, $saveandprint, $this->back_color, $this->fore_color,  $this->width,  $this->height);
        } catch (Exception $e) {
