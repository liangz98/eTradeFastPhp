<?php
class Shop_Controller_Action extends Zend_Controller_Action
{
	...
	{
		$rediM = new Kyapi_Model_redisInit();
		$userID=intval($_SESSION['rev_session']['userID']);
		$this ->view->userID=$userID;
//        $token = Seed_Cookie::getCookie('seed_Token');
//        $user_id = intval(Seed_Cookie::getCookie('seed_UserId'));
//		$this ->view->userId = $userId = Seed_Cookie::getCookie('seed_UserId');
...
//                //$mycarts = $cartM->fetchSum('goods_number',array('user_id'=>$user['user_id'],'shop_id'=>0,'is_gift'=>'0','goods_channel'=>$this->view->seed_Setting['goods_channel']));
//                $param = array();
