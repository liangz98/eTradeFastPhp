<?php
/**
 * 满送记录表模型 (snshop_goods_act_gifts)
 *
 * @category   Shop
 * @package    Shop_Model
 * @copyright  Copyright (c) 2014 萌芽网络 (http://www.gzseed.com)
 */
class Shop_Model_GoodsActGift extends Seed_Model_Db
{
    /**
     * 无前缀的表名
     *
     * @var string
     */
    public $_table_name = 'goods_act_gifts';
     ...
     *
     */
    public function fetchActGiftList($limit = array(0, 0), $condiction=null, $order_by=null, $fetch_fields="*", $debug=false)
    {
        try {
     ...
     *
     */
    public function fetchActGiftCount($condiction=null , $debug=false)
    {
        try {
            $select = $this->_db->select();
