 ...
 *
 **/
class Shop_Model_Branch extends Seed_Model_Db
{
	/**
     * 分类列表数组
     *
    ...
    public function fetchChildBranches($parent)
    {
        try {
            $select = $this->_db->select();
            $select->from($this->_prefix.$this->_table_name);
            ...
            foreach ($rows as $k=>$row)
            {
                $this->_branch_ids[] = $row['br_id'];
                $this->fetchChildBranches($row['br_id']);
            }
            if(is_array($this->_branch_ids))
                return array_reverse($this->_branch_ids);
            else
                return array();
        } catch (Exception $e) {
            throw $e;
