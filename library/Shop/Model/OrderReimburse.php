 ...
 */
class Shop_Model_OrderReimburse extends Seed_Model_Db
{
    /**
     * 无前缀的表名
     *
     * @var string
     */
    public $_table_name = 'order_reimburse';

    /* 订单退货状态 */
    const ORS_APPLY   = 0; //提交申请
    const ORS_ACCEPT  = 1; //接受申请
    const ORS_REFUSE  = 2; //拒绝申请
    const ORS_FINISH  = 3; //退换货完成
    const ORS_CANCEL  = 4; //用户取消
     ...
     *
     */
    function fetchReimburseList($limit = array(0,0) , $condiction=null , $order_by=null , $fetch_fields="*" , $debug=false){
        try {
            ...
            $select->joinLeft($this->_prefix."order_goods AS t3", 't1.rec_id = t3.rec_id', array('goods_name', 'goods_m_list_image'));
     ...
     *
     */
    function fetchReimburseCount($condiction=null , $debug=false){
        try {
            $select = $this->_db->select();
