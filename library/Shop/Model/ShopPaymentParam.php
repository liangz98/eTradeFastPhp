<?php
/**
 * 支付参数表模型 (snshop_payment_params)
 *
 * @category   Shop
 * @package    Shop_Model
 * @copyright  Copyright (c) 2014 萌芽网络 (http://www.gzseed.com)
 */
class Shop_Model_ShopPaymentParam extends Seed_Model_Db 
{
    /**
     * 无前缀的表名
     *
     * @var string
     */
    public $_table_name = 'shop_payment_params';
     ...
     *
     */
    public function fetchPaymentParam($condiction , $fetch_fields="*" , $order_by=null , $debug=false)
    {
        try {
            $select = $this->_db->select();
            $select->from($this->_prefix.$this->_table_name." as t1",$fetch_fields);
            $select->joinleft($this->_prefix."shop_payment_param_details AS t2", 't1.setting_id = t2.setting_id', array('shop_id', 'setting_content'));
     ...
     *
     */
    function fetchPaymentParams($limit = array(0,0) , $condiction=null , $order_by=null , $fetch_fields="*" , $debug=false){
        try {
            $select = $this->_db->select();
            $select->from($this->_prefix.$this->_table_name." as t1",$fetch_fields);
            $select->joinleft($this->_prefix."shop_payment_param_details AS t2", 't1.setting_id = t2.setting_id', array('shop_id', 'setting_content'));
     ...
     *
     */
    function fetchPaymentParamsCount($condiction=null , $debug=false){
        try {
            $select = $this->_db->select();
            $select->from($this->_prefix.$this->_table_name." as t1",$fetch_fields);
            $select->joinleft($this->_prefix."shop_payment_param_details AS t2", 't1.setting_id = t2.setting_id', array('shop_id', 'setting_content'));
