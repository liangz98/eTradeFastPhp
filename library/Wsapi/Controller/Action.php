<?php
//安卓api
class Wsapi_Controller_Action extends Zend_Controller_Action
{	
	function init()
	{
...
//					exit('权限不够，访问受限！');
                ...
                //验证身份
                $mall_id = intval($this->getRequest()->getParam('mall_id'));
                $wsapi_key = trim(strip_tags($this->getRequest()->getParam('wsapi')));
                if($mall_id < 1 || empty($wsapi_key)){
                    exit('params error');
                }
                $mallM = new Shop_Model_Mall('shop');
                $this->view->mall = $mall = $mallM->fetchRow(array('mall_id'=>$mall_id,'mall_key'=>$wsapi_key));
                if(empty($mall)){
                    exit('api interface invalid.');
                }
