...
class Wechat_Kefu_Image{
    ...
    function kefu(){
                ...
                if($wechat_user['ping_fakeid'] > 0){//对话
                                                 ...
                                                 if(is_array($materials)){
                                                     foreach($materials as $mVal){
                                                         if(isset($mVal['file_id']) && $mVal['file_id'] > 0 && isset($mVal['name']) && $mVal['name'] == $filename){
                                                             ...
                                                             if($pingWechatUser){
                                                                 $re = $WeiXinC->sendImage($pingWechatUser['fakeid'],$mVal['file_id']);//根据素材的id发送消息
                                                                 if(isset($re['base_resp']['err_msg'])){
                                                                     if(strtolower($re['base_resp']['err_msg']) == 'ok'){
                                                                           //发送成功
                                                                         $pic_url = $this->_wechat_base_controller->_wechat_base_params->PicUrl;
                                                                         Wechat_Kefu_RecordKefu::record($wechat_user['user_id'], $pingWechatUser['user_id'],"<a href='{$pic_url}' target='_blank'><img src='{$pic_url}' width='100' /></a>");
                         ...
                         //默认发送失败
                         $response->responseTextMsg('[系统消息]对不起，发送图片失败！');
