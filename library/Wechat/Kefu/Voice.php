         ...
         if($wechat_user['ping_fakeid'] > 0){//对话
                 ...
                 if($list){
                     ...
                     if(is_object($WeiXinC)){
                                         ...
                                         if(is_array($materials)){
                                                 foreach($materials as $mVal){
                                                     if(isset($mVal['file_id']) && $mVal['file_id'] > 0 && isset($mVal['name']) && $mVal['name'] == $filename){
                                                         ...
                                                         if($pingWechatUser){
                                                              $re = $WeiXinC->sendVoice($pingWechatUser['fakeid'], $mVal['file_id']);//根据素材的id发送消息
                                                                if(isset($re['base_resp']['err_msg'])){
                                                                      if(strtolower($re['base_resp']['err_msg']) == 'ok'){
                                                                              $rk_id = Wechat_Kefu_RecordKefu::record($wechat_user['user_id'], $pingWechatUser['user_id'],"语音消息");
                         ...
                         //默认发送失败
                         $response->responseTextMsg('[系统消息]对不起，发送语音失败！');
