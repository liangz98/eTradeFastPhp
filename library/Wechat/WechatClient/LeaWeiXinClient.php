 ...
 * 发送curl请求
 */
class Wechat_WechatClient_LeaWeiXinClient {
     ...
     */
    function submit($url, $data, $cookie = false ,$Referer = false) {
        $dataStr = "";
...
//        curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, 1); // 从证书中检查SSL加密算法是否存在
        ...
        if(!$Referer){
          $Referer = "https://mp.weixin.qq.com/cgi-bin/message?t=message/list&action=&keyword=&frommsgid=&offset=&count=&day=&token=&lang=zh_CN";
        }
        ...
        return array("cookie" => $cookie,'header'=>$header ,"body" =>$body);
    }
...
//        curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, 1); // 从证书中检查SSL加密算法是否存在
   ...
   function getSource($url, $cookie = false) {
            $curl = curl_init();
            curl_setopt($curl, CURLOPT_URL, $url); // 要访问的地址
            curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, 0); // 对认证证书来源的检查
//            curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, 1); // 从证书中检查SSL加密算法是否存在
