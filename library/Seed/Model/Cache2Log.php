...
class Seed_Model_Cache2Log {
	...
	{
		if(!defined('SEED_LOG_ROOT'))throw new Exception("SEED_LOG_ROOT not defined");
		$this->_postfix=".log.txt";
	}
...
//	 */
	public function save($file, $data){
		$file=SEED_LOG_ROOT."/".$file.date('Y-m-d-H').$this->_postfix;
...
//	 */
	...
	{
		$file=SEED_LOG_ROOT."/".$file.$this->_postfix;
		if(file_exists($file)){
			if(!@unlink($file)){
	...
	function get($file)
	{
		...
		if(file_exists($file)){
			require($file);
			return unserialize($seed_log_data);
		}else{
			return null;
		}
	}
	function find($file,$type,$key)
	{
		...
		if(file_exists($file)){
			require($file);
			$dataDD=unserialize($seed_log_data);
