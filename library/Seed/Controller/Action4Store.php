<?php
class Seed_Controller_Action4Store extends Zend_Controller_Action
{
	function init()
	{
		...
		$fromapi=false;
		if(empty($token)){
			$token = Seed_Cookie::getCookie('seed_StoreToken');
		}else{
			$fromapi=true;
		}
		...
		if($user['store_id']>0){
			...
			$store = $storeM->fetchRow(array("store_id"=>$user['store_id']));
		...
		if(!isset($setting['store_app_server']))$setting['store_app_server']="http://".$_SERVER['HTTP_HOST']."/store";
