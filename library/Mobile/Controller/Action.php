...
class Mobile_Controller_Action extends Zend_Controller_Action
{
	public function init()
	{
		...
		$this->view->addHelperPath(SEED_LIB_ROOT . '/Mobile/View/Helper');
    ...
    protected function initUser ()
    {
        ...
        if (is_array($my) && isset($my['user_id']) && $user_id == $my['user_id']) {
            ...
            if ($user) {
                //设置cookie
                $expiretime = time() +
                 $this->view->seed_Setting['cookie_expire'];
                Seed_Cookie::setCookie('seed_Token', $token, $expiretime, 
                $this->view->seed_Setting['cookie_path'], 
                $this->view->seed_Setting['cookie_host']);
                Seed_Cookie::setCookie('seed_UserId', $user['user_id'], 
                $expiretime, $this->view->seed_Setting['cookie_path'], 
                $this->view->seed_Setting['cookie_host']);
                Seed_Cookie::setCookie('seed_UserName', $user['user_name'], 
                $expiretime, $this->view->seed_Setting['cookie_path'], 
                $this->view->seed_Setting['cookie_host']);
