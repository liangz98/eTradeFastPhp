<?php
class Vmallhk_Controller_Action extends Zend_Controller_Action
{
	...
	{
		$shop_id = ($this->view->shop['shop_id']>0)?$this->view->shop['shop_id']:0;
        ...
        if(is_array($my) && isset($my['user_id']) && $user_id == $my['user_id']){
            ...
            if($user){
                ...
                Seed_Cookie::setCookie('seed_Token',$token,$expiretime,$this->view->seed_Setting['cookie_path'],$this->view->seed_Setting['cookie_host']);
                ...
                //$mycarts = $cartM->fetchSum('goods_number',array('user_id'=>$user['user_id'],'shop_id'=>$shop_id,'is_gift'=>'0','goods_channel'=>$this->view->seed_Setting['goods_channel']));
                $param = array();
                $param['user_id']=$user['user_id'];
                $param['goods_channel']=$this->view->seed_Setting['goods_channel'];
                $mycarts = $cartM->fetchCartCount($param);

                $this->view->total_carts = ($mycarts>0)?intval($mycarts):'0';
            }else{
                if($_COOKIE){
            ...
            $cartC = Seed_Cookie::getCookie('SeedCart');
            if ( !empty($cartC['session_id'])) {
                $cartM = new Shop_Model_Cart('shop');
                //$mycarts = $cartM->fetchSum('goods_number',array('user_id'=>0, 'shop_id'=>$shop_id, 'session_id'=>$cartC['session_id'],'is_gift'=>'0','goods_channel'=>$this->view->seed_Setting['goods_channel']));
                $param = array();
                $param['user_id']='0';
                $param['session_id']=$cartC['session_id'];
                $param['goods_channel']=$this->view->seed_Setting['goods_channel'];
                $mycarts = $cartM->fetchCartCount($param);
                $this->view->total_carts = ($mycarts>0)?intval($mycarts):'0';
            }
            ...
            if (file_exists($Autoload)) {
                ...
                foreach ($files as $file) {
                    if ($file != 'Autoload.php' &&
                     substr($file, strrpos($file, '.')) == '.php') {
                        $include_files[$file] = $common_dir . DIRECTORY_SEPARATOR .
                         $file;
                    }
                }
                if ($include_files) {
                    ...
                    foreach ($include_files as $key => $include_file) {
                        ...
                        $data_files[$key] = str_replace(SEED_LIB_ROOT, '', 
                        $include_file);
                    }
	...
	protected function initSetting()
	{
		...
		if(!isset($setting['vmall_app_server']))$setting['vmall_app_server']="/vmallhk";
		...
		$seed_v_tpl = trim($this->_request->getParam('tpl'));
		if(!empty($seed_v_tpl)){
			define('SEED_V_TPL',$seed_v_tpl);
		}else{
			if(defined('SEED_HOST_NAME')){
				$seed_host_name = str_replace(".","_",SEED_HOST_NAME);
